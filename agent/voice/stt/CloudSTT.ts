...
export class CloudSTT extends EventEmitter implements STTEngine {
  ...
  async #connectAzure() {
    ...
    this.#stream.onerror = () => this.#handleFatal(new Error('azure-websocket-error'));
    this.#stream.onclose = () => this.#handleFatal(new Error('azure-socket-closed'));
  }
  ...
  async #connectDeepgram() {
    ...
    this.#stream.onerror = () => this.#handleFatal(new Error('deepgram-websocket-error'));
    this.#stream.onclose = () => this.#handleFatal(new Error('deepgram-socket-closed'));
  }
  ...
  #startAudioStreaming() {
    ...
    this.#mediaRecorder.ondataavailable = (event) => {
      if (event.data.size > 0 && this.#stream && this.#stream.readyState === WebSocket.OPEN) {
  ...
  #parseAzureMessage(data: any): STTTranscript | null {
    try {
      ...
      if (parsed.RecognitionStatus === 'InitialSilenceTimeout' || 
      ...
      if (parsed.RecognitionStatus === 'Error') {
        throw new Error(`Azure STT error: ${parsed.ErrorMessage || 'Unknown error'}`);
      }

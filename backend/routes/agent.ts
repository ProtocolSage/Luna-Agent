...
export async function createAgentRouter(
  ...
  router.post("/chat", async (req: Request, res: Response) => {
    try {
      ...
      const memories = await vectorStore.search(cleanMessage, {
        limit: 10,
        type: "document",
      });
      ...
      let toolResult = null;
      if (useTools) {
        toolResult = await toolOrchestrator.processWithTools(cleanMessage, {
          sessionId,
        });
      }
  ...
  router.post("/execute-tool", async (req: Request, res: Response) => {
    try {
      ...
      const result = await toolExecutive.executePlan(
        [{ tool, args }],
        sessionId || "direct",
      );
  ...
  router.post("/civil", (req, res) => {
    ...
    return res.json({
      civilMode,
      message: `Civil mode ${civilMode ? "enabled" : "disabled"}`,
    });
  });
  ...
  router.post("/task/complete", (req, res) => {
    ...
    return res.json({
      success: ok,
      message: ok ? "Task marked as complete" : "Task not found",
    });
  });

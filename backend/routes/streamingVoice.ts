...
function initializeVoiceWebSocket(server: Server) {
  ...
  console.log(
    "[StreamingVoice] WebSocket server initialized on /ws/voice/stream",
  );
    ...
    try {
      ...
      ws.on("message", async (message) => {
        try {
          ...
          console.error(
            `[StreamingVoice] Message error in session ${sessionId}:`,
            error,
          );
      ...
      ws.on("error", (error) => {
        console.error(
          `[StreamingVoice] WebSocket error in session ${sessionId}:`,
          error,
        );
      });
      ...
      console.error(
        `[StreamingVoice] Failed to initialize session ${sessionId}:`,
        error,
      );
 ...
 * Forward voice service events to WebSocket client
 */
function setupVoiceServiceForwarding(
  service: StreamingVoiceService,
  ws: WebSocket,
  sessionId: string,
) {
...
router.get("/status", (req: Request, res: Response) => {
  const activeSessions = Array.from(activeVoiceSessions.entries()).map(
    ([id, session]) => ({
      sessionId: id,
...
router.get("/sessions", (req: Request, res: Response) => {
  const sessions = Array.from(activeVoiceSessions.entries()).map(
    ([id, session]) => ({
      sessionId: id,
      state: session.service.getState(),
...
router.delete("/sessions/:sessionId", async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error(
      `[StreamingVoice] Failed to terminate session ${sessionId}:`,
      error,
    );
...
router.post("/broadcast", (req: Request, res: Response) => {
  ...
  for (const [sessionId, session] of activeVoiceSessions.entries()) {
    try {
      ...
      console.error(
        `[StreamingVoice] Failed to broadcast to session ${sessionId}:`,
        error,
      );
      errorCount++;
    }

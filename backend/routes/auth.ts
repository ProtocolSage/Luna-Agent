...
router.get('/validate', async (req, res) => {
  try {
    ...
    if (!sid) return res.status(401).json({ success: false, valid: false, error: 'missing-session-id' });

    const session = sessions.get(sid);
    if (!session) return res.status(401).json({ success: false, valid: false, error: 'invalid-session-id' });

    // Check if session is expired (30 minutes)
    const thirtyMinutesAgo = new Date(Date.now() - 30 * 60 * 1000);
    if (session.lastAccessed < thirtyMinutesAgo) {
      sessions.delete(sid);
      return res.status(401).json({ success: false, valid: false, error: 'session-expired' });
    }

...
describe('Preload Security - Secret Scrubbing', () => {
  ...
  describe('API Key Exposure Prevention', () => {
    ...
    it('should NOT expose ANTHROPIC_API_KEY to renderer', () => {
      expect(preloadCode).not.toMatch(/ANTHROPIC_API_KEY:\s*process\.env\.ANTHROPIC_API_KEY/);
    });

    it('should NOT expose AZURE_SPEECH_KEY to renderer', () => {
      expect(preloadCode).not.toMatch(/AZURE_SPEECH_KEY:\s*process\.env\.AZURE_SPEECH_KEY/);
    });

    it('should NOT expose DEEPGRAM_API_KEY to renderer', () => {
      expect(preloadCode).not.toMatch(/DEEPGRAM_API_KEY:\s*process\.env\.DEEPGRAM_API_KEY/);
    });

    it('should NOT expose GOOGLE_CLOUD_API_KEY to renderer', () => {
      expect(preloadCode).not.toMatch(/GOOGLE_CLOUD_API_KEY:\s*process\.env\.GOOGLE_CLOUD_API_KEY/);
    });

    it('should NOT expose ELEVEN_API_KEY to renderer', () => {
      expect(preloadCode).not.toMatch(/ELEVEN_API_KEY:\s*process\.env\.ELEVEN_API_KEY/);
    });

    it('should NOT expose PICOVOICE_ACCESS_KEY to renderer', () => {
      expect(preloadCode).not.toMatch(/PICOVOICE_ACCESS_KEY:\s*process\.env\.PICOVOICE_ACCESS_KEY/);
    });
  ...
  describe('Safe Configuration Values', () => {
    it('should expose LUNA_API_BASE (non-sensitive)', () => {
      expect(preloadCode).toContain('LUNA_API_BASE: process.env.LUNA_API_BASE');
    });

    it('should expose API_BASE (non-sensitive)', () => {
      expect(preloadCode).toContain('API_BASE: process.env.API_BASE');
    });

    it('should expose feature flags (non-sensitive)', () => {
      expect(preloadCode).toContain('VOICE_AUTO_LISTEN');
      expect(preloadCode).toContain('WAKE_WORD_ENABLED');
      expect(preloadCode).toContain('VOICE_ENABLED');
    });

    it('should expose STT_PROVIDER config (non-sensitive)', () => {
      expect(preloadCode).toContain("STT_PROVIDER: process.env.STT_PROVIDER || 'azure'");
    });

    it('should expose WAKE_WORD config (non-sensitive)', () => {
      expect(preloadCode).toContain("WAKE_WORD: process.env.WAKE_WORD || 'luna'");
    });

    it('should expose AZURE_SPEECH_REGION (non-sensitive config)', () => {
      expect(preloadCode).toContain('AZURE_SPEECH_REGION: process.env.AZURE_SPEECH_REGION');
    });
  });

...
class VoiceInput extends events_1.EventEmitter {
        ...
        switch (provider) {
            case 'webSpeech':
                if (!this.isWebSpeechAvailable()) {
                    ...
                    const { NotSupportedSTTProvider } = await Promise.resolve().then(() => require('./notSupportedSTTProvider'));
    ...
    async startRecording() {
        ...
        if (this.currentProvider === 'webspeech' || this.currentProvider === 'webSpeech') {
    ...
    async startAudioRecording() {
        try {
            ...
            this.emit('error', new Error(`Microphone access failed: ${error.message}`));
        }
    }
    ...
    async transcribeWithAzure(audioBuffer) {
        try {
            ...
            const response = await axios_1.default.post(`https://${region}.stt.speech.microsoft.com/speech/recognition/conversation/cognitiveservices/v1`, audioBuffer, {
                headers: {
    ...
    async transcribeWithGoogle(audioBuffer) {
        try {
            ...
            const response = await axios_1.default.post(`https://speech.googleapis.com/v1/speech:recognize?key=${apiKey}`, {
                config: {
                    encoding: 'WEBM_OPUS',

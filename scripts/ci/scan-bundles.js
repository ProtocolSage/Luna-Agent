...
const SECRET_PATTERNS = [
  ...
  { name: 'ElevenLabs API Key', pattern: /[a-f0-9]{32}/g, severity: 'HIGH', context: 'ELEVEN' },
  { name: 'Picovoice Access Key', pattern: /[A-Za-z0-9+/]{88}==/g, severity: 'HIGH', context: 'PICOVOICE' },
  ...
  { name: 'SSH Key', pattern: /-----BEGIN OPENSSH PRIVATE KEY-----/g, severity: 'CRITICAL' },

  // Environment variable leaks
  { name: 'Hardcoded Password', pattern: /password["\s:=]+["'][^"'\s]{8,}["']/gi, severity: 'HIGH' },
  { name: 'Database URL', pattern: /postgres(ql)?:\/\/[^"'\s]+/gi, severity: 'HIGH' },
  { name: 'MongoDB URL', pattern: /mongodb(\+srv)?:\/\/[^"'\s]+/gi, severity: 'HIGH' },
...
class SecretScanner {
  ...
  isAllowlisted(match, context) {
    return ALLOWLIST.some(pattern =>
      pattern.test(match) || pattern.test(context)
    );
  }
  ...
  scanFile(filePath) {
    ...
    try {
      ...
      SECRET_PATTERNS.forEach(({ name, pattern, severity, context: requiredContext }) => {
        ...
        for (const match of matches) {
          ...
          if (requiredContext && !context.toUpperCase().includes(requiredContext)) {
            continue;
          }
  ...
  printReport() {
    ...
    console.log('3. Add secrets to .gitignore and .env.example (with placeholders)');

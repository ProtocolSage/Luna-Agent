...
class ToolExecutive {
    constructor(policy) {
        this.tools = new Map();
        this.goalManager = new goals_1.GoalManager();
        this.reminderManager = new reminders_1.ReminderManager();
        this.policy = policy || { allowlist: [] };
        this.registerAll();
    }
    registerTool(def) { this.tools.set(def.name, def); }
    getToolDefinitions() { return Array.from(this.tools.values()); }
    getToolDefinitionsAsText() {
        return Array.from(this.tools.values()).map(tool => `${tool.name}: ${tool.description} (params: ${JSON.stringify(tool.parameters)})`);
    }
    async executePlan(steps, traceId) {
        var _a;
        const context = {
            traceId,
            sessionId: traceId.split('_')[1] || 'unknown',
            workingDir: process.cwd(),
            allowlist: ((_a = this.policy) === null || _a === void 0 ? void 0 : _a.allowlist) || [],
        };
        const results = [];
    ...
    registerAll() {
        ...
        this.registerTool({
            ...
            handler: async (args) => { assertSafePath(args.path); return fs.readFile(args.path, 'utf8'); }
        });
        this.registerTool({
            name: 'write_file',
            ...
            handler: async (args) => { assertSafePath(args.path); await fs.writeFile(args.path, args.content, 'utf8'); return 'OK'; }
        });
        this.registerTool({
            name: 'append_file',
            description: 'Append string to file.',
            parameters: { path: { type: 'string', required: true }, content: { type: 'string', required: true } },
            handler: async (args) => { assertSafePath(args.path); await fs.appendFile(args.path, args.content, 'utf8'); return 'OK'; }
        });
        this.registerTool({
            name: 'move_file',
            description: 'Move/rename a file.',
            parameters: { src: { type: 'string', required: true }, dest: { type: 'string', required: true } },
            handler: async (args) => { assertSafePath(args.src); assertSafePath(args.dest); await fs.rename(args.src, args.dest); return 'OK'; }
        });
        this.registerTool({
            name: 'delete_file',
            description: 'Delete a file.',
            parameters: { path: { type: 'string', required: true } },
            handler: async (args) => { assertSafePath(args.path); await fs.unlink(args.path); return 'OK'; }
        });
        this.registerTool({
            name: 'copy_file',
            description: 'Copy file.',
            parameters: { src: { type: 'string', required: true }, dest: { type: 'string', required: true } },
            handler: async (args) => { assertSafePath(args.src); assertSafePath(args.dest); await fs.copyFile(args.src, args.dest); return 'OK'; }
        });
        this.registerTool({
            name: 'list_directory',
        ...
        this.registerTool({
            ...
            parameters: { path: { type: 'string', required: true } },
            handler: async (args) => { assertSafePath(args.path); return fs.stat(args.path); }
        });
        // ---- Process/Command Execution ----
        this.registerTool({
            name: 'execute_command',
            description: 'Run shell command and return stdout (sync).',
            parameters: { command: { type: 'string', required: true }, cwd: { type: 'string', default: process.cwd() } },
            handler: async (args) => {
                if (!args.command)
                    throw new Error('command required');
                return (0, child_process_1.execSync)(args.command, { cwd: args.cwd || process.cwd(), encoding: 'utf8', timeout: 30000 });
            }
        });
        this.registerTool({
            name: 'execute_python',
            ...
            handler: async (args, ctx) => {
                ...
                try {
                    return (0, child_process_1.execSync)(`python "${fname}"`, { encoding: 'utf8', timeout: 30000 });
                }
                finally {
        ...
        this.registerTool({
            ...
            handler: async (args, ctx) => {
                ...
                await fs.writeFile(fname, args.code, 'utf8');
                try {
                    return (0, child_process_1.execSync)(`node "${fname}"`, { encoding: 'utf8', timeout: 30000 });
                }
                finally {
                    await fs.unlink(fname).catch(() => { });
                }
        ...
        this.registerTool({
            ...
            handler: async (args) => {
                ...
                try {
                    const res = await (0, node_fetch_1.default)(args.url, { signal: controller.signal });
                    if (!res.ok)
        ...
        this.registerTool({
            ...
            parameters: { url: { type: 'string', required: true }, dest: { type: 'string', required: true } },
            handler: async (args) => {
                const controller = new AbortController();
        ...
        this.registerTool({
            ...
            parameters: { path: { type: 'string', required: true }, url: { type: 'string', required: true } },
            handler: async (args) => {
                assertSafePath(args.path);
                ...
                const timeout = setTimeout(() => controller.abort(), 30000);
                try {
                    const res = await (0, node_fetch_1.default)(args.url, { method: 'POST', body: data, signal: controller.signal });
                    if (!res.ok)
                        throw new Error(`HTTP ${res.status}`);
                    return await res.text();
                }
        ...
        this.registerTool({
            ...
            parameters: { query: { type: 'string', required: true }, limit: { type: 'number', default: 5 } },
            handler: async (args) => {
                const q = encodeURIComponent(args.query);
                const controller = new AbortController();
                const timeout = setTimeout(() => controller.abort(), 10000);
                try {
                    const html = await (await (0, node_fetch_1.default)(`https://html.duckduckgo.com/html/?q=${q}`, { signal: controller.signal })).text();
                    const links = Array.from(html.matchAll(/<a[^>]+class="[^"]*result__a[^"]*"[^>]+href="([^"]+)"/g)).map(x => x[1]);
        ...
        this.registerTool({
            ...
            handler: async (args) => {
                ...
                const timeout = setTimeout(() => controller.abort(), 10000);
                try {
                    const html = await (await (0, node_fetch_1.default)(args.url, { signal: controller.signal })).text();
        ...
        this.registerTool({
            ...
            handler: async (args) => {
                ...
                try {
                    const html = await (await (0, node_fetch_1.default)(args.url, { signal: controller.signal })).text();
                    const $ = cheerio.load(html);
                    return $('a[href]').map((_, e) => $(e).attr('href')).get();
                }
                finally {
                    clearTimeout(timeout);
                }
        ...
        this.registerTool({
            ...
            handler: async () => {
                if (process.platform === 'win32')
                    return (0, child_process_1.execSync)('powershell Get-Clipboard', { encoding: 'utf8' });
                if (process.platform === 'darwin')
                    return (0, child_process_1.execSync)('pbpaste', { encoding: 'utf8' });
                return (0, child_process_1.execSync)('xclip -selection clipboard -o', { encoding: 'utf8' });
            }
        });
        this.registerTool({
            ...
            handler: async (args) => {
                if (process.platform === 'win32')
                    (0, child_process_1.execSync)(`echo "${args.content.replace(/"/g, '\\"')}" | clip`);
                else if (process.platform === 'darwin')
                    (0, child_process_1.execSync)(`echo "${args.content.replace(/"/g, '\\"')}" | pbcopy`);
                else
                    (0, child_process_1.execSync)(`echo "${args.content.replace(/"/g, '\\"')}" | xclip -selection clipboard`);
                return 'OK';
            }
        });
        ...
        this.registerTool({
            ...
            handler: async () => {
                if (process.platform === 'win32') {
                    const out = (0, child_process_1.execSync)('powershell "Get-Process | Where-Object {$_.MainWindowTitle} | Select-Object MainWindowTitle"', { encoding: 'utf8' });
                    return out.split('\n').map(x => x.trim()).filter(x => x && x !== 'MainWindowTitle');
                }
        ...
        this.registerTool({
            ...
            parameters: {},
            handler: async () => ({ platform: process.platform, arch: process.arch, node: process.version })
        });
        this.registerTool({
            name: 'status',
            ...
            parameters: {},
            handler: async () => {
                return await (0, status_1.getStatus)();
            }
        });
        this.registerTool({
            name: 'get_env_var',
        ...
        this.registerTool({
            ...
            parameters: { name: { type: 'string', required: true }, value: { type: 'string', required: true } },
            handler: async (args) => { process.env[args.name] = args.value; return 'OK'; }
        });
        this.registerTool({
            name: 'list_env_vars',
        ...
        this.registerTool({
            ...
            handler: async (args) => { await new Promise(r => setTimeout(r, args.ms)); return 'OK'; }
        });
        // ---- Memory & Knowledge Management ----
        this.registerTool({
            name: 'add_memory',
            description: 'Add a new memory with content, type, and optional metadata.',
            parameters: {
            ...
            handler: async (args) => {
                const validTypes = ['conversation', 'document', 'goal', 'reminder', 'journal', 'note', 'task'];
                if (!validTypes.includes(args.type)) {
                    throw new Error(`Invalid memory type. Valid types: ${validTypes.join(', ')}`);
                }
                const memory = await memoryService.addMemory(args.content, args.type, args.metadata);
                return {
                    id: memory.id,
        ...
        this.registerTool({
            name: 'search_memory',
            description: 'Search memories by content. Uses vector search if embeddings available.',
            parameters: {
        ...
        this.registerTool({
            ...
            handler: async (args) => {
                ...
                let memories;
                if (args.type) {
                    memories = await memoryService.getMemoriesByType(args.type, limit, offset);
                }
                else {
        ...
        this.registerTool({
            ...
            description: 'Find memories similar to given content using vector search.',
            parameters: {
                content: { type: 'string', required: true },
        ...
        this.registerTool({
            ...
            parameters: { ms: { type: 'number', required: true } },
            handler: async (args) => { setTimeout(() => { }, args.ms); return 'OK'; }
        });
        this.registerTool({
            name: 'set_interval',
            description: 'Set interval callback (dummy in CLI context).',
            parameters: { ms: { type: 'number', required: true } },
            handler: async (args) => { setInterval(() => { }, args.ms); return 'OK'; }
        });
        this.registerTool({
            name: 'cancel_task',
            description: 'Cancel a scheduled task (stub).',
            parameters: { id: { type: 'string', required: true } },
            handler: async (_args) => { /* implement if you wire up timers */ return 'OK'; }
        });
    }
}
exports.ToolExecutive = ToolExecutive;

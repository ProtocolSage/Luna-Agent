...
if (!API_KEY)
    throw new Error('Set ELEVEN_API_KEY user env-var first!');
...
class VoiceEngine {
    ...
    async fetchAndDecode(text) {
        try {
            const response = await axios_1.default.post(`https://api.elevenlabs.io/v1/text-to-speech/${currentVoiceId}`, {
                text,
    ...
    async streamToSpeaker(text, signal) {
        ...
        try {
            ...
            const response = await axios_1.default.post(`https://api.elevenlabs.io/v1/text-to-speech/${currentVoiceId}/stream`, {
                text,
                model_id: 'eleven_monolingual_v1',
                voice_settings: {
                    stability: 0.5,

...
export interface Memory {
  id: string
  content: string
  type: 'conversation' | 'document' | 'goal' | 'reminder' | 'journal' | 'note' | 'task'
  timestamp: string
  embedding?: number[]
...
class SupabaseManager {
  ...
  private initializeFromEnv(): void {
    try {
      ...
      if (url && anonKey) {
        ...
        console.warn('[Supabase] No configuration found in environment variables')
        console.log('[Supabase] Set SUPABASE_URL and SUPABASE_ANON_KEY to enable cloud database')
      }
    } catch (error) {
  ...
  async testConnection(): Promise<boolean> {
    ...
    try {
      const { data, error } = await this.client.from('memories').select('count', { count: 'exact', head: true })
  ...
  async setupSchema(): Promise<void> {
    ...
    console.log('[Supabase] Schema setup complete. Please run the following SQL in your Supabase dashboard:')

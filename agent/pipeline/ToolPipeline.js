...
`;
        try {
            const response = await this.modelRouter.generateResponse('You are an expert tool execution planner.', planningPrompt, {
    ...
    async executeStepWithRetry(step, context, signal) {
        var _a;
        let lastError = null;
        const maxRetries = (_a = this.config.retryCount) !== null && _a !== void 0 ? _a : 2;
...
`;
            const response = await this.modelRouter.generateResponse(synthesisPrompt, { temperature: 0.7, maxTokens: 2000 });
            return {
    ...
    async logPipelineStart(request, context) {
        await MemoryService_1.memoryService.addMemory(`Pipeline started: ${request}`, 'task', {
            type: 'pipeline_start',
    ...
    async logPipelineResult(result, context) {
        await MemoryService_1.memoryService.addMemory(`Pipeline completed: ${result.success ? 'SUCCESS' : 'FAILED'} (${result.totalTimeMs}ms)`, 'task', {
    ...
    async logPipelineError(error, context) {
        await MemoryService_1.memoryService.addMemory(`Pipeline error: ${error.message}`, 'task', {
            type: 'pipeline_error',
            traceId: context.traceId,
            sessionId: context.sessionId,
            error: error.message,

...
export class VoiceInputService extends EventEmitter {
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      ...
      if (!this.openaiApiKey) {
        console.log(
          "[VoiceInputService] No OpenAI API key - using Web Speech API fallback",
        );
      } else {
  ...
  private async loadDependencies(): Promise<void> {
    try {
      ...
      console.log(
        "[VoiceInputService] Dependencies loaded for wake word detection",
      );
    } catch (error) {
  ...
  private async initializeWakeWord(): Promise<void> {
    try {
      ...
      if (!accessKey) {
        console.log(
          "[VoiceInputService] Wake word detection disabled - no Picovoice key",
        );
        return;
      }
      ...
      try {
        PorcupineNode = (await import(
          "@picovoice/porcupine-node"
        )) as PorcupineModule;
      } catch (error) {
        console.log(
          "[VoiceInputService] Porcupine Node SDK not available:",
      ...
      if (fs.existsSync(keywordPath)) {
        ...
        console.log(
          "[VoiceInputService] Wake word file not found:",
          keywordPath,
        );
      }
    } catch (error) {
      console.error(
        "[VoiceInputService] Wake word initialization failed:",
        error,
      );
  ...
  private async startWakeWordDetection(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.log(
          "[VoiceInputService] Mic module not available for wake word detection",
        );
        return;
      }
      ...
      inputStream.on("data", (data: Buffer) => {
        ...
        try {
          ...
          console.error(
            "[VoiceInputService] Wake word processing error:",
            error,
          );
        }
      });
      ...
      inputStream.on("error", (error: Error) => {
        console.error(
          "[VoiceInputService] Wake word audio stream error:",
          error,
        );
      });
      ...
      console.error(
        "[VoiceInputService] Failed to start wake word detection:",
        error,
      );
    }
  }
  ...
  private initializeWebSpeech(): void {
    console.log(
      "[VoiceInputService] Initialized in fallback mode - using Web Speech API through renderer",
    );
  ...
  public async startListening(): Promise<void> {
    ...
    if (!this.openaiApiKey) {
      ...
      console.log(
        "[VoiceInputService] Using browser speech recognition fallback",
      );
  ...
  private async startAudioCapture(): Promise<void> {
    try {
      ...
      try {
        ...
        console.log(
          "[VoiceInputService] Mic module not available, using browser fallback",
        );
        this.emit("request-browser-speech");
        return;
      }
      ...
      if (!mic) {
        console.log(
          "[VoiceInputService] Failed to load mic module, using browser fallback",
        );
        this.emit("request-browser-speech");
        return;
      }
      ...
      console.log(
        "[VoiceInputService] Starting audio capture for Whisper STT...",
      );
      ...
      inputStream.on("silence", async () => {
        console.log(
          "[VoiceInputService] Silence detected, processing audio...",
        );
      ...
      console.error(
        "[VoiceInputService] Failed to start audio capture:",
        error,
      );
      throw error;
    }

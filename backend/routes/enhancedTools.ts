...
router.post('/execute', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.log(`🔧 Executing tool: ${tool} for session: ${sessionId || 'anonymous'}`);
...
router.post('/plan', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.log(`🎯 Executing tool plan: "${request}" for session: ${sessionId}`);
...
router.post('/submit', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.log(`📥 Submitting to pipeline: "${request}" for session: ${sessionId}`);
 ...
 */
router.get('/result/:executionId', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (result) {
      ...
      res.json({
        ...
        executionId,
        status,
        message: status === 'not_found' ? 'Execution not found' : `Execution is ${status}`,
        timestamp: new Date().toISOString()
      });
    }
...
router.get('/list', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json({
      ...
      categories: {
        filesystem: tools.filter(t => t.name.includes('file') || t.name.includes('directory')).length,
        network: tools.filter(t => t.name.includes('fetch') || t.name.includes('web') || t.name.includes('scrape')).length,
        memory: tools.filter(t => t.name.includes('memory')).length,
        system: tools.filter(t => t.name.includes('system') || t.name === 'status').length,
        productivity: tools.filter(t => t.name.includes('goal') || t.name.includes('reminder')).length
      },
      timestamp: new Date().toISOString()
    });
 ...
 */
router.delete('/cancel/:executionId', async (req: Request, res: Response): Promise<void> => {
  try {
    const { executionId } = req.params;

...
class StreamingAudioProcessor extends AudioWorkletProcessor {
  ...
  updateVAD(sample) {
    ...
    this.currentVadLevel =
      this.currentVadLevel * (1 - this.vadSmoothingFactor) +
  ...
  getVADConfidence() {
    ...
    const avgVAD =
      this.vadHistory.reduce((sum, val) => sum + val, 0) /
      this.vadHistory.length;
  ...
  handleMessage(event) {
    ...
    switch (type) {
      ...
      case "updateNoiseGate":
        ...
        console.log(
          "[AudioWorklet] Noise gate updated:",
      ...
      case "updateNormalization":
        ...
        console.log(
          "[AudioWorklet] Normalization target updated:",
          this.targetLevel,
        );
        break;

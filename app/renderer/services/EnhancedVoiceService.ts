...
export class EnhancedVoiceService extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.emit(
        "error",
        `Enhanced voice service initialization failed: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  private async setupEnhancedAudioProcessing(): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to setup enhanced audio processing: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async startEnhancedListening(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to start enhanced listening: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async setupAudioAnalysis(stream: MediaStream): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to setup audio analysis: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private processVAD(): void {
    ...
    if (speechDetected) {
      ...
      if (
        this.currentMode === "voice-activity" &&
        !this.baseVoiceService.isListeningActive
      ) {
  ...
  private cleanupAudioNodes(): void {
    try {
      ...
      console.warn(
        "[EnhancedVoiceService] Error cleaning up audio nodes:",
        error,
      );
    }
  }

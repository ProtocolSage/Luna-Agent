...
describe("Memory System Integration Tests", () => {
  ...
  async function runTool(toolName: string, args: Record<string, any>) {
    const results = await toolExec.executePlan(
      [{ tool: toolName, args }],
      "integration_test",
    );
  ...
  describe("End-to-End Memory Flow", () => {
    test("should complete full memory lifecycle: add → search → update → delete", async () => {
      ...
      expect(addResult.content).toBe(
        "Integration test memory content for e2e testing",
      );
      ...
      expect(updateResult.content).toBe(
        "Updated integration test memory content",
      );
      ...
      await expect(runTool("get_memory", { id: testMemoryId })).rejects.toThrow(
        `Memory not found: ${testMemoryId}`,
      );
  ...
  describe("Error Handling and Edge Cases", () => {
    test("should handle invalid memory operations gracefully", async () => {
      ...
      await expect(
        runTool("get_memory", { id: "non_existent_id" }),
      ).rejects.toThrow("Memory not found: non_existent_id");

      // Test deleting non-existent memory
      await expect(
        runTool("delete_memory", { id: "non_existent_id" }),
      ).rejects.toThrow("Memory not found: non_existent_id");

...
describe('Electron Security Configuration', () => {
  ...
  describe('Content Security Policy', () => {
    ...
    it('should only allow localhost connections', () => {
      const cspMatch = mainProcessCode.match(/connect-src.*?['"]([^'"]+)['"]/);
      if (cspMatch) {
        const connectSrc = cspMatch[1];
        // Should only allow localhost, not wildcard domains
        expect(connectSrc).toMatch(/localhost|127\.0\.0\.1/);
  ...
  describe('Media Permissions for Voice', () => {
    ...
    it('should configure autoplay policy for TTS', () => {
      expect(mainProcessCode).toContain("autoplayPolicy: 'no-user-gesture-required'");
    });
  ...
  describe('Security Regression Prevention', () => {
    ...
    it('should NOT have nodeIntegration: true', () => {
      const nodeIntegrationMatches = mainProcessCode.match(/nodeIntegration:\s*true/g);
      expect(nodeIntegrationMatches).toBeNull();
    });

    it('should NOT have contextIsolation: false', () => {
      const contextIsolationMatches = mainProcessCode.match(/contextIsolation:\s*false/g);
  ...
  describe('Electron Module Loading', () => {
    it('should safely load electron module', () => {
      ...
      expect(mainProcessCode).toContain("require('electron')");
      expect(mainProcessCode).toMatch(/typeof electron === 'string'/);
    });

    it('should verify app.whenReady is a function', () => {
      expect(mainProcessCode).toContain("typeof app.whenReady !== 'function'");
    });
...
describe('Bootstrap Script Security', () => {
  const bootstrapScriptPath = path.join(__dirname, '../../scripts/write-bootstrap.cjs');

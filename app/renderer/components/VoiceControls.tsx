  ...
  useEffect(() => {
    ...
    if (window.stt) {
      ...
      unsubscribeTranscript = window.stt.onTranscript(
        ({ text, isFinal }: { text: string; isFinal: boolean }) => {
          console.log("[VoiceControls] Hybrid STT transcript:", {
            text,
            isFinal,
          });
    ...
    if (window.voiceIPC) {
      if (typeof window.voiceIPC.onSttTranscript === "function") {
        window.voiceIPC.onSttTranscript(
      ...
      if (typeof window.voiceIPC.onMicPermission === "function") {
        window.voiceIPC.onMicPermission((data: { granted: boolean }) => {
          if (!data.granted) {
            ...
            setState((prev) => ({
              ...prev,
              error: prev.error?.includes("Microphone") ? null : prev.error,
            }));
          }
        });
    ...
    return () => {
      ...
      if (typeof offIpcListeningStarted === "function")
        offIpcListeningStarted();
      if (typeof offIpcListeningStopped === "function")
        offIpcListeningStopped();
    ...
    const handleError = (error: string) => {
      ...
      if (error.includes("network") || error.includes("Speech recognition")) {
        ...
        setTimeout(() => {
          if (window.stt) {
            console.log(
              "VoiceControls: Starting Hybrid STT after Web Speech failure",
            );
      ...
      if (state.isListening) {
        ...
        if (window.stt) {
          ...
          if (!result.success) {
            throw new Error(
              result.error ||
                "Hybrid STT failed to start - check cloud API keys or Whisper installation",
            );
          }
          // Don't set listening state manually - let hybrid STT events handle it
        } else {
          throw new Error(
            "Hybrid STT system not available. Please check configuration and restart Luna Agent.",
          );
        }
      }
    } catch (error) {
          ...
          <div className="error-actions">
            ...
            {state.webSpeechFailed && (
              <span
                ...
                title="Web Speech API failed - using Hybrid STT instead"
              >
                Using backup STT
              </span>
            )}

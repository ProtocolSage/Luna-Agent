 ...
 */
export async function exampleVoiceConversation(userText: string, sessionId?: string) {
  try {
 ...
 */
export async function exampleMicrophoneFlow(audioBlob: Blob, sessionId?: string) {
  try {
...
export async function exampleAdvancedTTS(text: string) {
  ...
  for (const options of ttsOptions) {
    try {
      ...
      console.log(`TTS successful with provider: ${options.provider || 'auto'}`);
      return player;
 ...
 */
export async function exampleToolExecution(toolName: string, input: any, sessionId?: string) {
  try {

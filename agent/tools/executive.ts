...
export class ToolExecutive {
  ...
  registerTool(def: ToolDefinition) { this.tools.set(def.name, def); }
  getToolDefinitions(): ToolDefinition[] { return Array.from(this.tools.values()); }
  ...
  private registerAll() {
    ...
    this.registerTool({
      ...
      handler: async (args) => {
        ...
        return execSync(args.command, { cwd: args.cwd || process.cwd(), encoding: 'utf8', timeout: 30000 });
      }
    });
    this.registerTool({
      name: 'execute_python',
      ...
      handler: async (args, ctx) => {
        ...
        try {
          return execSync(`python "${fname}"`, { encoding: 'utf8', timeout: 30000 });
        } finally { await fs.unlink(fname).catch(() => {}); }
      }
    });
    this.registerTool({
      ...
      handler: async (args, ctx) => {
        ...
        await fs.writeFile(fname, args.code, 'utf8');
        try {
          return execSync(`node "${fname}"`, { encoding: 'utf8', timeout: 30000 });
        } finally { await fs.unlink(fname).catch(() => {}); }
      }
    });
    ...
    this.registerTool({
      ...
      handler: async (args) => {
        ...
        const timeout = setTimeout(() => controller.abort(), 30000);
        try {
          const res = await fetch(args.url, { method: 'POST', body: data, signal: controller.signal });
          if (!res.ok) throw new Error(`HTTP ${res.status}`);
          return await res.text();
        } finally {
          clearTimeout(timeout);
    ...
    this.registerTool({
      ...
      handler: async (args) => {
        ...
        const timeout = setTimeout(() => controller.abort(), 10000);
        try {
          const html = await (await fetch(`https://html.duckduckgo.com/html/?q=${q}`, { signal: controller.signal })).text();
    ...
    this.registerTool({
      ...
      handler: async (args) => {
        ...
        const timeout = setTimeout(() => controller.abort(), 10000);
        try {
          const html = await (await fetch(args.url, { signal: controller.signal })).text();
    ...
    this.registerTool({
      ...
      handler: async (args) => {
        ...
        try {
          const html = await (await fetch(args.url, { signal: controller.signal })).text();
          const $ = cheerio.load(html);
          return $('a[href]').map((_: any, e: any) => $(e).attr('href')).get();
        } finally {
          clearTimeout(timeout);
        }
      }
    ...
    this.registerTool({
      ...
      parameters: {},
      handler: async () => {
        if (process.platform === 'win32') return execSync('powershell Get-Clipboard', { encoding: 'utf8' });
    ...
    this.registerTool({
      ...
      handler: async (args) => {
        if (process.platform === 'win32') execSync(`echo "${args.content.replace(/"/g, '\\"')}" | clip`);
        else if (process.platform === 'darwin') execSync(`echo "${args.content.replace(/"/g, '\\"')}" | pbcopy`);
        else execSync(`echo "${args.content.replace(/"/g, '\\"')}" | xclip -selection clipboard`);
        return 'OK';
      }
    });
    ...
    this.registerTool({
      ...
      handler: async () => {
        if (process.platform === 'win32') {
          const out = execSync('powershell "Get-Process | Where-Object {$_.MainWindowTitle} | Select-Object MainWindowTitle"', { encoding: 'utf8' });
    ...
    this.registerTool({
      ...
      handler: async (args) => {
        ...
        let memories;
        if (args.type) {
          memories = await memoryService.getMemoriesByType(args.type as MemoryType, limit, offset);
        } else {

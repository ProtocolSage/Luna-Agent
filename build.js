...
class LunaBuildSystem {
  constructor() {
    ...
    console.log(
      `${this.isWatching ? "👁️  Watch mode enabled" : "🔨 Single build mode"}\n`,
    );
  }
  ...
  async build() {
    try {
      ...
      await Promise.all([this.buildMainProcess(), this.buildRendererProcess()]);
  ...
  async preBuildChecks() {
    ...
    if (!this.compareVersions(nodeVersion.slice(1), requiredNode)) {
      throw new Error(
        `Node.js ${requiredNode}+ required, found ${nodeVersion}`,
      );
    }
  ...
  async verifyDependencies() {
    try {
      ...
      const criticalDeps = ["electron", "react", "typescript", "webpack"];
  ...
  async copyStaticAssets() {
    ...
    const assetsToCopy = [
      ...
      {
        src: "app/renderer/public",
        dest: "dist/app/renderer/public",
        recursive: true,
        optional: true,
      },
    ];
  ...
  getRendererProcessConfig() {
    ...
    return {
      ...
      devtool:
        this.mode === BUILD_MODES.production ? "source-map" : "eval-source-map",
    };
  }
...
self.addEventListener('fetch', event => {
  ...
  async generateBuildManifest() {
    ...
    const manifest = {
      ...
      integrity:
        this.mode === BUILD_MODES.production
          ? JSON.parse(
              fs.readFileSync(path.join("dist", "integrity.json"), "utf8"),
            )
          : null,
    };
  ...
  copyRecursive(src, dest, recursive = false) {
    ...
    if (stat.isDirectory() && recursive) {
      ...
      for (const file of files) {
        this.copyRecursive(path.join(src, file), path.join(dest, file), true);
      }
    } else if (stat.isFile()) {

...
type InChannels =
  ...
  | "voice:processing-stopped"
  | "voice:tts-audio-data"
  | "voice:tts-started"
  | "voice:tts-stopped"
  ...
  | "voice:push-to-talk-pressed"
  | "voice:push-to-talk-released"
  | "stt:transcript"
  | "stt:engine-switched"
...
contextBridge.exposeInMainWorld("__ENV", {
  ...
  LUNA_CONTINUOUS_CONVERSATION:
    process.env.LUNA_CONTINUOUS_CONVERSATION === "true",
  LUNA_AUTO_LISTEN_AFTER_TTS: process.env.LUNA_AUTO_LISTEN_AFTER_TTS === "true",
  LUNA_SILENCE_TIMEOUT: parseInt(
    process.env.LUNA_SILENCE_TIMEOUT || "3000",
    10,
  ),
...
contextBridge.exposeInMainWorld("voiceIPC", {
  ...
  invoke: <T = unknown>(ch: HandleChannels, ...args: any[]) =>
    ipcRenderer.invoke(ch, ...args),

  // Specific voice methods for compatibility
  speak: (text: string, options?: any) =>
    ipcRenderer.invoke("voice:tts-speak", text, options),
  chatWithTTS: (message: string) =>
    ipcRenderer.invoke("voice:chat-with-tts", message),
...
contextBridge.exposeInMainWorld("stt", {
  ...
  onTranscript: (
    ...
    const handler = (_: any, payload: { text: string; isFinal: boolean }) =>
      cb(payload);
    ipcRenderer.on("stt:transcript", handler);
    return () => ipcRenderer.removeListener("stt:transcript", handler);
  },
  onEngineSwitch: (
    cb: (info: { engine: string; isCloud: boolean }) => void,
  ) => {
    const handler = (_: any, info: { engine: string; isCloud: boolean }) =>
      cb(info);

...
export function initializeToolSystem(router?: ModelRouter): void {
  ...
  try {
    ...
    console.log(
      `🛠️  Available tools: ${toolExecutive.getToolDefinitions().length}`,
    );
  } catch (error) {
...
export function getPipelineService(): PipelineService {
  if (!pipelineService) {
    console.log(
      "⚠️  PipelineService not initialized, creating with defaults...",
    );
    initializeToolSystem();
  }
...
export async function executeToolPlan(
  ...
  try {
    ...
    const result = await pipeline.executeWithOrchestration(
      userRequest,
      id,
      context,
    );

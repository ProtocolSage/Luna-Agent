...
export class ModelRouter {
  ...
  private async callOpenAI(
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        temperature:
          (options.temperature as number) || model.temperature || 0.7,
  ...
  private async checkOllamaAvailability(
    ...
    if (!modelExists) {
      throw new Error(
        `Model ${modelName} not found. Available models: ${availableModels.map((m: any) => m.name).join(", ")}`,
      );
    }
  }
   ...
   */
  async routeWithOfflineFallback(
    prompt: string,
    options: {
      temperature?: number;
      maxTokens?: number;

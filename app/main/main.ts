...
try {
  ...
  if (typeof electron === "string") {
    console.log(
      "[Main] Electron resolved to path string, loading index.js directly...",
    );
...
const logger = {
  ...
  error: (msg: string, error?: Error, category: string = "") =>
...
class LunaMainProcess {
  ...
  private createMainWindow(): void {
    ...
    try {
      ...
      logger.warn("Failed to register CSP header injection", "main-process", {
    ...
    if (rendererUrl) {
      ...
      console.log(
        "[main-process] Loading renderer from dev server:",
        rendererUrl,
      );
      this.mainWindow?.loadURL(
        `${rendererUrl}?apiBase=${encodeURIComponent(apiBase)}`,
      );
    } else {
      ...
      if (!require("fs").existsSync(rendererFile)) {
        console.error(
          "[main-process] Renderer file not found! Run npm run build:renderer first",
        );
        dialog.showErrorBox(
          "Renderer Not Built",
  ...
  private async startBackendServer(): Promise<void> {
    ...
    if (!isDev) {
      ...
      if (fs.existsSync(serverPath)) {
        ...
        console.warn(
          "[Main] App will try to connect to external backend at localhost:3001",
        );
      }
    } else {
      // Development: Backend started externally
      logger.info(
        "Backend server should be started externally (npm run backend)",
        "main-process",
      );
    }
    return;
  ...
  private registerIpcHandlers(): void {
    ...
    ipcMain.handle(
      ...
      (
        _event: IpcMainInvokeEvent,
        options: { title?: string; body?: string; [key: string]: any },
      ) => {
  ...
  private loadWindowState(): void {
    try {
      ...
      logger.warn("Failed to load window state", "main-process", {
        error: error instanceof Error ? error.message : "Unknown error",
      });
    }
  }
  ...
  private saveWindowState(): void {
    try {
      ...
      logger.warn("Failed to save window state", "main-process", {
        error: error instanceof Error ? error.message : "Unknown error",
      });
    }
  }

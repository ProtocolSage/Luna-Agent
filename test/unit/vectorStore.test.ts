...
describe('VectorStore', () => {
  ...
  describe('similarity method', () => {
    ...
    it('should perform text-based similarity search without API key', async () => {
      ...
      expect(results.every(r => r.document.content.toLowerCase().includes('machine'))).toBe(true);
    });
    ...
    it('should perform vector similarity with mocked embeddings', async () => {
      ...
      const foundDoc = results.find(r => r.document.id === 'doc-with-embedding');
  ...
  describe('search method', () => {
    ...
    it('should filter by type', async () => {
      ...
      expect(results.length).toBeGreaterThan(0);
      expect(results.every(r => r.document.type === 'conversation')).toBe(true);
    });

    it('should filter by sessionId', async () => {
      const results = await vectorStore.search('AI', { sessionId: 'session-1' });
      expect(results.length).toBeGreaterThan(0);
      expect(results.every(r => r.document.sessionId === 'session-1')).toBe(true);
    });
    ...
    it('should combine filters', async () => {
      ...
      expect(results.length).toBeGreaterThan(0);
      expect(results.every(r => r.document.type === 'conversation' && r.document.sessionId === 'session-1')).toBe(true);
    });
  });
});

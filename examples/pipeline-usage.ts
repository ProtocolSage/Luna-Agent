  ...
  try {
    ...
    if (result.steps.length > 0) {
      ...
      result.steps.forEach((step, i) => {
        console.log(`  ${i + 1}. ${step.tool} - ${step.success ? 'SUCCESS' : 'FAILED'}`);
  ...
  try {
    const result = await reasoningEngine.reason(
      ...
      
      {
        ...
        userId: "example_user",
        availableTools: [
          'write_file', 'read_file', 'list_directory', 'stat_file'
        ],
    ...
    if (result.toolCalls && result.toolCalls.length > 0) {
      ...
      result.toolCalls.forEach((call, i) => {
        console.log(`  ${i + 1}. ${call.tool} - ${call.success ? 'SUCCESS' : 'FAILED'} (${call.latencyMs}ms)`);
      });
    }
    ...
    if (result.steps && result.steps.length > 0) {
      ...
      result.steps.forEach((step, i) => {
        ...
        if (step.observation) console.log(`    Result: ${step.observation.substring(0, 100)}...`);
      });
    }
    ...
    const result = await pipeline.execute(
      ...
      },
      {
        ...
        providedSteps: [
          ...
          { tool: 'analyze_code_quality', args: { filePath: 'example.ts', language: 'typescript' } },
          { tool: 'write_file', args: { path: 'code_analysis_report.md', content: 'Generated report content...' } }
        ]
      }
    );
...
async function runExamples() {
  ...
  console.log('Note: Make sure you have the required environment variables set:');

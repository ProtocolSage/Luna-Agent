...
import {
  addMemory,
  memAdd as memAddCompat,
  memSearch as memSearchCompat,
} from "../services/api/memoryClient";
...
const FuturisticUI: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "system",
      content:
        "Neural interface initialized. Ready for hyperdimensional processing...",
      timestamp: new Date(),
    },
  ]);
  ...
  const applyComputeMode = useCallback(
    (mode: "gpu" | "cpu", persist = false) => {
  ...
  const initNeuralNetwork = useCallback(() => {
    ...
    for (let layer = 0; layer < nodes.length - 1; layer++) {
      for (let i = 0; i < nodes[layer].length; i++) {
        for (let j = 0; j < nodes[layer + 1].length; j++) {
          if (Math.random() > 0.3) {
            const points = [
              nodes[layer][i].position,
              nodes[layer + 1][j].position,
            ];
    ...
    const animate = () => {
      ...
      nodes.forEach((layer, layerIndex) => {
        layer.forEach((node, nodeIndex) => {
          const scale =
            1 + Math.sin(Date.now() * 0.001 + layerIndex + nodeIndex) * 0.2;
    ...
    return () => {
      // cleanup
      if ((rendererRef as any).currentRaf)
        cancelAnimationFrame((rendererRef as any).currentRaf);
      renderer.dispose();
      scene.clear();
    };
  }, [metrics.gpuMode]);
    ...
    async (audioBlob: Blob): Promise<string> => {
      try {
        ...
        try {
          ...
          console.warn(
            "New STT client failed, trying voiceClient transcribe:",
            newTranscribeError,
          );
          ...
          try {
            ...
            console.warn(
              "Voice client transcribe failed, trying legacy client:",
              voiceClientError,
            );
        ...
        if (
          transcript.includes("dummy transcription") ||
          transcript.includes("This is a dummy")
        ) {
          throw new Error(
            "Dummy transcription detected - forcing real Whisper",
          );
        }
  ...
  const initSpeechRecognition = useCallback(() => {
    ...
    try {
      ...
      if (SpeechRecognition) {
        ...
        const recognition =
          new SpeechRecognition() as SpeechRecognitionInterface;
    ...
    if (voiceStatus.isListening) {
      ...
      if (voiceStatus.useWhisper) {
        startWhisperRecording().catch((error) => {
          console.warn(
            "Whisper failed, falling back to Web Speech API:",
            error,
          );
          ...
          if (recognitionRef.current) {
            setVoiceStatus((p) => ({
              ...p,
              useWhisper: false,
              error: "Using Web Speech API fallback",
            }));
            try {
              ...
              setVoiceStatus((p) => ({
                ...p,
                error: "Both voice recognition methods failed",
              }));
            }
          } else {
            setVoiceStatus((p) => ({
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.warn(
            "New memory client failed, using compatibility functions:",
            newMemError,
          );
      ...
      const eMsg: Message = {
        id: `e-${Date.now()}`,
        role: "system",
        content: `Neural link disrupted. (${err?.message || "Unknown error"})`,
        timestamp: new Date(),
      };
  ...
  const generateTTS = async (text: string) => {
    try {
      ...
      try {
        ...
        console.warn(
          "TTS failed, falling back to Web Speech:",
          err?.message || err,
        );
        ...
        try {
          ...
          setVoiceStatus((p) => ({ ...p, isSpeaking: false }));
          toastErr(
            `All TTS options failed: ${speechErr?.message || "Unknown error"}`,
          );
        }
      }
    } catch (err: any) {
  ...
  const checkVoiceProviders = async () => {
    try {
      ...
      if (!data.availableProviders?.length)
        setVoiceStatus((p) => ({
          ...p,
          error: "No voice providers available",
        }));
    } catch {
      setVoiceStatus((p) => ({ ...p, error: "Voice service unavailable" }));
    }
  };
  ...
  useEffect(() => {
    (window as any).toggleGpu =
      (window as any).__toggleGpu || (() => onToggleGpu());
    (window as any).toggleMatrix = (checked: boolean) =>
      onToggleMatrix(checked);
  }, [onToggleGpu]);
  ...
  return (
    <div
      className={`futuristic-ui ${partyMode ? "party-mode" : ""} ${!matrixOn ? "matrix-off" : ""} ${metrics.gpuMode ? "" : "cpu-mode"}`}
    >
      {/* Background */}
      ...
      <div className="main-container">
        ...
        <header className="header">
          <div className="logo glitch" data-text="LUNA PRO">
            LUNA PRO
          </div>
          ...
          <div className="performance-metrics">
            ...
            <div className="metric">
              ...
              <span className="metric-value">
                {typeof metrics.tokensPerSec === "number"
                  ? Math.round(metrics.tokensPerSec).toLocaleString()
                  : "—"}
              </span>
            </div>
            <div className="metric">
            ...
            <label className="toggle matrix-toggle" title="Toggle Matrix Rain">
              <input
                ...
                onChange={(e) =>
                  (window as any).toggleMatrix?.(e.target.checked)
                }
              />
            ...
            <label
              className="toggle voice-toggle"
              title="Auto-transmit after voice input"
            >
              <input
                type="checkbox"
            ...
            >
              ...
              {message.role === "assistant" && (
                <div className="message-actions">
                  ...
                  <button className="mini-btn" onClick={regenerateFromLast}>
                    Regenerate
                  </button>
                </div>
              )}
          ...
          <div className="frequency-bars" ref={barsContainerRef}>
            {Array.from({ length: metrics.gpuMode ? 48 : 24 }, (_, i) => (
              <div
                ...
                style={
                  {
                    ["--bar-height" as any]: `${20 + Math.random() * 60}%`,
        ...
        <div className="input-container">
          ...
          <button
            ...
            title={
              voiceStatus.useWhisper
                ? "Primary: Whisper STT (Superior)"

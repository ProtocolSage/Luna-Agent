...
export class LunaVoiceAgent {
  ...
  private playSound(type: 'wake' | 'error' | 'success') {
    const sounds = {
      ...
      error: 'data:audio/wav;base64,UklGRl4CAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YToCAAA=',
      success: 'data:audio/wav;base64,UklGRl4CAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YToCAAA='
    };
  ...
  private onUserStartedSpeaking() {
    window.dispatchEvent(new CustomEvent('luna-user-speaking', { detail: true }));
  }

  private onUserStoppedSpeaking() {
    window.dispatchEvent(new CustomEvent('luna-user-speaking', { detail: false }));
  }

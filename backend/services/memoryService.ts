...
const f32ToBuf = (v: Float32Array) => Buffer.from(v.buffer, v.byteOffset, v.byteLength);
const bufToF32 = (b?: Buffer) => (b && b.length) ? new Float32Array(new Uint8Array(b).buffer) : new Float32Array();

export async function addMemory(content: string, type: string, sessionId?: string, metadata: any = {}) {
  const db = getDB();
  const id = randomUUID();
  ...
  `).run({ id, content, embedding: f32ToBuf(vec), metadata: JSON.stringify(metadata), ts, type });
...
function safeJSON(s: any) { try { return s ? JSON.parse(s) : {}; } catch { return {}; } }

...
export class StreamingVoiceService extends EventEmitter {
  ...
  constructor(apiKey?: string) {
    ...
    this.interruptHandler = new InterruptHandler(
      this.config.interruptThreshold,
    );
  ...
  private sendSessionConfig(): void {
    ...
    const config = {
      ...
      session: {
        ...
        instructions:
          "You are Luna, a helpful AI voice assistant. Speak naturally and conversationally. Be concise but engaging.",
        voice: "alloy",
  ...
  private processAudioData(audioData: {
    ...
    const cleanAudio = this.echoSuppressor.process(
      inputBuffer,
    ...
    if (this.state.isSpeaking && hasVoice) {
      const shouldInterrupt = this.interruptHandler.shouldInterrupt(
        cleanAudio,
        timestamp,
      );
      if (shouldInterrupt) {
...
class VADDetector {
  ...
  detect(audioData: Float32Array, vadLevel?: number): boolean {
    if (vadLevel !== undefined) {
      ...
      this.currentLevel =
        this.currentLevel * (1 - this.smoothingFactor) +
        vadLevel * this.smoothingFactor;
    } else {
      // Compute RMS energy
      let sum = 0;

...
class BetterSQLiteDatabase {
  ...
  private init(): void {
    try {
      ...
      console.log(
        `[BetterSQLiteDB] Database initialized: ${this.config.filename}`,
      );
    } catch (error) {
      ...
      all(...params: any[]) {
        ...
        if (sqlLower.includes("select")) {
          ...
          if (match) {
            ...
            const rows = self.data.get(tableName) || [];
            console.log(
              `[InMemoryDB] SELECT from ${tableName}, found ${rows.length} rows`,
            );
            return rows;
          }
        }
  ...
  try {
    ...
    if (!Database) {
      try {
        ...
        console.warn(
          "[Database] better-sqlite3 not available:",
    ...
    console.warn(
      "[Database] Failed to initialize persistent database, falling back to in-memory:",

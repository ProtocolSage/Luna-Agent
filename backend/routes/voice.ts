...
const elevenLabsService = process.env.ELEVEN_API_KEY
...
const normaliseProviderError = (
  ...
  const didTimeout = timeoutSignatures.some(
    (signature) =>
  ...
  try {
    ...
    if (!provider || provider === "elevenlabs") {
      if (!elevenLabsService) {
        ...
        try {
          const audioStream = await elevenLabsService.fetchAudioStream(
            text,
            voiceId,
          ...
          console.warn(
            "ElevenLabs TTS failed, trying OpenAI fallback:",
    ...
    if (process.env.OPENAI_API_KEY && (provider === "openai" || !provider)) {
      try {
        ...
        if (provider === "openai") {
          res
            .status(500)
            .json({ error: "OpenAI TTS failed", details: openaiError.message });
          return;
        }
      }
    }
...
export const handleTranscribe = async (
  ...
  try {
    ...
    const originalExt = path
      ...
      .toLowerCase();
    const mimeSubtype =
      (req.file.mimetype || "").split("/").pop()?.toLowerCase() || "";
    const isAllowedAudio =
      allowedAudioExtensions.has(originalExt) ||
      allowedAudioExtensions.has(mimeSubtype);

...
class ModelRouter {
    ...
    getAvailableModels() {
        return this.models.filter(model => {
            ...
            if (breaker.state === 'OPEN') {
                const lastFailure = (_a = breaker.lastFailureTime) !== null && _a !== void 0 ? _a : 0;
        ...
        for (let attempt = 0; attempt < this.MAX_RETRIES; attempt++) {
            try {
                ...
                if (attempt < this.MAX_RETRIES - 1) {
                    const delay = Math.min(this.BASE_DELAY * Math.pow(2, attempt) + Math.random() * 1000, this.MAX_DELAY);
    ...
    async callOpenAI(model, prompt, options) {
        ...
        return {
            ...
            tokensUsed: ((_a = data.usage) === null || _a === void 0 ? void 0 : _a.total_tokens) || 0,
    ...
    async callAnthropic(model, prompt, options) {
        ...
        return {
            ...
            tokensUsed: (((_a = data.usage) === null || _a === void 0 ? void 0 : _a.input_tokens) || 0) + (((_b = data.usage) === null || _b === void 0 ? void 0 : _b.output_tokens) || 0),
    ...
    async checkOllamaAvailability(modelName, baseUrl) {
        ...
        const modelExists = availableModels.some((model) => model.name === modelName || model.name.startsWith(modelName + ':'));
        if (!modelExists) {
            throw new Error(`Model ${modelName} not found. Available models: ${availableModels.map((m) => m.name).join(', ')}`);
        }
    }
    /**

...
export class VoiceInput extends EventEmitter {
    ...
    switch (provider) {
      case 'webSpeech':
        if (!this.isWebSpeechAvailable()) {
          ...
          const { NotSupportedSTTProvider } = await import('./notSupportedSTTProvider');
  ...
  private async startAudioRecording(): Promise<void> {
    try {
      ...
      this.emit('error', new Error(`Microphone access failed: ${(error as Error).message}`));
    }
  }
  ...
  private async transcribeWithWhisper(audioBuffer: ArrayBuffer): Promise<string> {
    try {
  ...
  private async transcribeWithGoogle(audioBuffer: ArrayBuffer): Promise<string> {
    try {

...
export class WakeWordService {
  ...
  async initialize(accessKey?: string): Promise<boolean> {
    ...
    if (accessKey && await this.checkPorcupineAssets()) {
      try {
        ...
        logger.info('Porcupine assets found, but using Web Speech API for simplicity');
        return true;
      } catch (error) {
  ...
  stop(): void {
    ...
    try {
      ...
      logger.debug('Error stopping recognition (may be already stopped):', error);
    }
  }
...
export function getWakeWordService(options?: WakeWordOptions): WakeWordService {
  ...
  if (!serviceInstance) {
    throw new Error('WakeWordService not initialized. Please provide options on first call.');
  }

...
(async () => {
  ...
  try {
    ...
    await voiceService.speak('And now I am Charlie - a different voice entirely.');
    ...
    voiceService.speak('This is HIGH PRIORITY and should play first!', { priority: 10 });
    ...
    voiceService.speak('This is a long message that will be interrupted in two seconds...', { priority: 1 });
    
    setTimeout(() => {
      console.log('⏹️ Interrupting with high priority message!');
      voiceService.speak('INTERRUPT! This is an urgent message!', { interrupt: true, priority: 100 });
    }, 2000);
    ...
    await voiceService.speak('And we are back to Nova! All features tested successfully.');
    
    // Test 8: Queue status
    console.log('8️⃣ Queue status:', voiceService.getQueueStatus());
    console.log('Available voices:', Object.keys(voiceService.getAvailableVoices()));

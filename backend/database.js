      ...
      return {
        run: (...params) => {
          try {
            ...
            if (sqlLower.includes('insert')) {
              const match = sqlLower.match(/insert(?:\s+or\s+replace)?\s+into\s+(\w+)/);
              if (match) {
                const tableName = match[1];
                ...
                if (tableName === 'memory_vectors') {
                  row.id = params[0] || `mock_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
                  ...
                  row.id = params[0] || `mem_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
                  row.content = params[1] || '';
            ...
            if (sqlLower.includes('update')) {
              ...
              if (match) {
                ...
                if (sqlLower.includes('where id = ?') && params[params.length - 1]) {
                  ...
                  if (rowIndex >= 0) {
                    ...
                    if (tableName === 'memories') {
                      ...
                      if (params.length >= 5) {
                        ...
                        console.log(`[InMemoryDB] UPDATE ${tableName} id=${idToUpdate}:`, rows[rowIndex]);
                        return { changes: 1 };
                      }
            ...
            if (sqlLower.includes('delete from')) {
              ...
              if (match) {
                ...
                if (sqlLower.includes('where id = ?') && params[0]) {
                  ...
                  console.log(`[InMemoryDB] DELETE from ${tableName} where id = ${params[0]}`);
        ...
        get: (...params) => {
          try {
            ...
            if (sqlLower.includes('select min(') || sqlLower.includes('select max(')) {
              const match = sqlLower.match(/from\s+(\w+)/);
              if (match) {
                const tableName = match[1];
                const rows = self.data.get(tableName) || [];
                ...
                const timestamps = rows.map(row => row.timestamp).filter(t => t);
                timestamps.sort();
            ...
            if (sqlLower.includes('select') && sqlLower.includes('where id = ?')) {
              ...
              if (match && params[0]) {
                ...
                console.log(`[InMemoryDB] SELECT from ${tableName} where id = ${params[0]}:`, found || null);
                return found || null;
              }
            }
              ...
              if (match) {
                ...
                if (sqlLower.includes('where')) {
                  ...
                  if (sqlLower.includes('sessionid = ?')) {
                    rows = rows.filter(row => row.sessionId === params[paramIndex]);
                    paramIndex++;
                  }
                }
                
                console.log(`[InMemoryDB] SELECT all from ${tableName}:`, rows.length, 'rows');
                return rows;
              }
            }
            return [];

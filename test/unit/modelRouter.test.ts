...
describe('ModelRouter', () => {
  ...
  describe('route method', () => {
    it('should throw error when no models available', async () => {
      ...
      await expect(emptyRouter.route('test')).rejects.toThrow('No models available');
    });
    ...
    it('should handle API errors gracefully', async () => {
      const mockResponse = {
        ...
        text: jest.fn().mockResolvedValue('{"error": {"message": "Invalid API key"}}')
      };

      (global.fetch as jest.Mock).mockResolvedValue(mockResponse);

      await expect(modelRouter.route('test')).rejects.toThrow('OpenAI API error');
    });
  });
  ...
  describe('cost calculation', () => {
    ...
    it('should return 0 for undefined usage', () => {
      const cost = modelRouter.calculateOpenAICost('gpt-4o-2024-08-06', undefined);
      expect(cost).toBe(0);
    });
  });

...
export class SupabaseMemoryStore {
    ...
    try {
      ...
      if (this.client && supabaseManager.isReady()) {
        ...
        if (this.isOnline) {
          ...
          console.warn(
            "[SupabaseMemoryStore] Cloud database unavailable, running in offline mode",
          );
        }
      } else {
        console.log(
          "[SupabaseMemoryStore] Supabase not configured, using offline mode only",
        );
      }
    } catch (error) {
  ...
  async addMemory(memory: Omit<Memory, "id" | "timestamp">): Promise<Memory> {
    ...
    if (this.isOnline && this.client) {
      ...
      this.queueOfflineAction("add", fullMemory);
      console.log(
        "[SupabaseMemoryStore] Memory queued for sync (offline mode)",
      );
      return fullMemory;
    }
  }
  ...
  async updateMemory(
    ...
    if (this.isOnline && this.client) {
      try {
        ...
        if (error) {
          console.error(
            "[SupabaseMemoryStore] Failed to update memory:",
            error,
          );
      ...
      this.queueOfflineAction("update", { id, updates });
      console.log(
        "[SupabaseMemoryStore] Memory update queued for sync (offline mode)",
      );
      return null;
    }
  }
  ...
  async deleteMemory(id: string): Promise<boolean> {
    if (this.isOnline && this.client) {
      try {
        ...
        if (error) {
          console.error(
            "[SupabaseMemoryStore] Failed to delete memory:",
            error,
          );
      ...
      this.queueOfflineAction("delete", { id });
      console.log(
        "[SupabaseMemoryStore] Memory deletion queued for sync (offline mode)",
      );
  ...
  async getMemoryById(id: string): Promise<Memory | null> {
    if (this.isOnline && this.client) {
      ...
      console.warn(
        "[SupabaseMemoryStore] Cannot retrieve memory in offline mode",
      );
      return null;
    }
  }
  ...
  async getMemoriesByType(
    ...
    if (this.isOnline && this.client) {
      try {
        ...
        if (error) {
          console.error(
            "[SupabaseMemoryStore] Failed to get memories by type:",
            error,
          );
          return [];
        }
        ...
        console.error(
          "[SupabaseMemoryStore] Get memories by type error:",
          error,
        );
        return [];
      }
      ...
      console.warn(
        "[SupabaseMemoryStore] Cannot retrieve memories in offline mode",
      );
      return [];
    }
  }
  ...
  async getRecentMemories(limit = 20, offset = 0): Promise<Memory[]> {
    if (this.isOnline && this.client) {
      try {
        ...
        if (error) {
          console.error(
            "[SupabaseMemoryStore] Failed to get recent memories:",
            error,
          );
          return [];
        }
        ...
        console.error(
          "[SupabaseMemoryStore] Get recent memories error:",
          error,
        );
        return [];
      }
  ...
  async searchMemories(
    ...
    if (!this.isOnline || !this.client) {
      console.warn(
        "[SupabaseMemoryStore] Cannot search memories in offline mode",
      );
      return [];
    }
    ...
    try {
      ...
      if (error) {
        console.error(
          "[SupabaseMemoryStore] Failed to search memories:",
          error,
        );
        return [];
      }
  ...
  async vectorSearch(
    ...
    if (!this.isOnline || !this.client) {
      console.warn(
        "[SupabaseMemoryStore] Cannot perform vector search in offline mode",
      );
      return [];
    }
  ...
  async close(): Promise<void> {
    if (this.offlineQueue.length > 0) {
      console.log(
        `[SupabaseMemoryStore] Syncing ${this.offlineQueue.length} offline operations...`,
      );
      await this.processOfflineQueue();
    }
  ...
  private queueOfflineAction(action: string, data: any): void {
    ...
    console.log(
      `[SupabaseMemoryStore] Queued ${action} operation for offline sync`,
    );
  }
  ...
  private async processOfflineQueue(): Promise<void> {
    ...
    console.log(
      `[SupabaseMemoryStore] Processing ${this.offlineQueue.length} offline operations...`,
    );
    ...
    for (const operation of queue) {
      try {
        ...
        console.error(
          `[SupabaseMemoryStore] Failed to process offline ${operation.action}:`,
          error,
        );
    ...
    if (this.offlineQueue.length === 0) {
      console.log(
        "[SupabaseMemoryStore] All offline operations synced successfully",
      );
    } else {
      console.warn(
        `[SupabaseMemoryStore] ${this.offlineQueue.length} operations failed to sync`,
      );
    }
  }
}

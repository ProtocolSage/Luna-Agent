  ...
  const argv = yargs(hideBin(process.argv))
    .command(
      ...
      async () => {
        ...
        defs.forEach(def => {
          const params = Object.keys(def.parameters).length > 0 
    ...
    .command(
      ...
      (y) => {
        ...
        y.positional('type', { type: 'string', describe: 'Memory type (conversation|document|goal|reminder|journal|note|task)' });
        y.option('metadata', { type: 'string', describe: 'JSON metadata object' });
      },
      async (argv) => {
        const metadata = argv.metadata ? JSON.parse(argv.metadata as string) : undefined;
    ...
    .command(
      ...
      (y) => {
        ...
        y.option('limit', { type: 'number', default: 10, describe: 'Number of results' });
      },
      async (argv) => {
        const result = await toolExec.executePlan([{
          tool: 'search_memory',
        ...
        if (result[0].success) {
          ...
          memories.forEach((mem: any, i: number) => {
            ...
            if (mem.similarity) console.log(`   🎯 Similarity: ${(mem.similarity * 100).toFixed(1)}%`);
          });
        } else {
    ...
    .command(
      ...
      (y) => {
        y.option('type', { type: 'string', describe: 'Filter by memory type' });
        y.option('limit', { type: 'number', default: 20, describe: 'Number of memories' });
      },
      async (argv) => {
        const result = await toolExec.executePlan([{
          tool: 'list_memories',
        ...
        if (result[0].success) {
          ...
          memories.forEach((mem: any, i: number) => {
            ...
            console.log(`   📅 ${mem.timestamp} ${mem.hasEmbedding ? '🧠' : '📝'}`);
          });
        } else {

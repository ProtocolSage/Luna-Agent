...
export class AzureSTTProvider implements STTProvider {
  ...
  private handleWebSocketMessage(event: MessageEvent): void {
    try {
      ...
      if (data.RecognitionStatus === "Success") {
        ...
        this.emit(
          "error",
          `Azure Speech recognition error: ${data.ErrorMessage || "Unknown error"}`,
        );
      }
    } catch (error) {
  ...
  private attemptReconnect(): void {
    ...
    console.log(
      `Azure STT: Attempting reconnection ${this.reconnectAttempts}/${this.maxReconnectAttempts}`,
    );
      ...
      if (this._isListening) {
        try {
          ...
          if (this.reconnectAttempts >= this.maxReconnectAttempts) {
            this.emit(
              "service-unavailable",
              "Azure Speech service unavailable after multiple reconnection attempts",
            );
            this.stopListening();
          }
        }
    ...
    if (listeners) {
      listeners.forEach((listener) => {
        try {
          ...
          console.error(
            `Error in Azure STT event listener for ${event}:`,
            error,
          );
        }
      });

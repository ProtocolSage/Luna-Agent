...
function migrate(dbPath) {
  ...
  if (!hasCol(db, "memories", "sessionId"))
    db.exec(`ALTER TABLE memories ADD COLUMN sessionId TEXT;`);
  ...
  if (c === 0)
    db.exec(
      `INSERT INTO memories_fts(id, content, type, timestamp) SELECT id, content, type, timestamp FROM memories;`,
    );

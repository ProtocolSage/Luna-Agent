...
// Target directories
const DIST_ASSETS_DIR = path.join(__dirname, '..', 'dist', 'app', 'renderer', 'assets');
const PUBLIC_ASSETS_DIR = path.join(__dirname, '..', 'app', 'renderer', 'public', 'assets');
...
function extractWasmFromWorker(workerPath) {
  try {
    ...
    const wasmBase64Pattern = /const\s+wasmBase64\s*=\s*["']([A-Za-z0-9+/=]+)["']/;
    ...
    if (match && match[1]) {
      ...
      fs.writeFileSync(path.join(PUBLIC_ASSETS_DIR, 'pv_porcupine.wasm'), wasmBuffer);
...
if (!fs.existsSync(path.join(DIST_ASSETS_DIR, keywordFile))) {
  ...
  fs.writeFileSync(path.join(PUBLIC_ASSETS_DIR, keywordFile), placeholderKeyword);
...
if (foundWorker) {  console.log('✅ Worker file extracted');
} else {

...
export function getEnvironmentConfig(
  environment: keyof typeof VOICE_ENVIRONMENT_CONFIGS,
): EnhancedVoiceConfig {
...
export async function testEnvironmentAndRecommend(): Promise<{
  ...
  try {
    ...
    const noiseVariation = Math.sqrt(
      samples.reduce((sum, val) => sum + Math.pow(val - avgNoise, 2), 0) /
        samples.length,
    );
    ...
    if (avgNoise > 30) {
      ...
      reasons.push(
        `Moderate noise environment detected (${avgNoise.toFixed(1)})`,
      );
...
export function loadSavedVoiceConfig(): EnhancedVoiceConfig | null {
  try {
    ...
    if (saved) {
      ...
      if (
        config.timestamp &&
        Date.now() - config.timestamp < 30 * 24 * 60 * 60 * 1000
      ) {
    ...
    console.warn(
      "[VoiceConfig] Failed to load saved voice configuration:",
      error,
    );
  }
  return null;

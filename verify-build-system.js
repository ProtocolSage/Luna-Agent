...
async function main() {
  ...
  allGood &= checkFile('scripts/build-renderer.js', 'Production renderer build');
  allGood &= checkFile('webpack.config.js', 'Backend config only');

  // Check unused files are properly marked
  log('\n🚫 Unused Files Status:', 'blue');
  allGood &= checkFile('webpack.renderer.UNUSED.js', 'Unused webpack renderer (marked)');
  allGood &= checkFile('webpack.renderer.config.UNUSED.js', 'Unused webpack config (marked)');

  // Check build outputs after production build
  log('\n🏗️  Testing Production Build:', 'blue');
  const buildSuccess = await runCommand('npm run build:renderer', 'Production renderer build');
  
  if (buildSuccess) {
    log('\n📦 Build Outputs:', 'blue');
    allGood &= checkFile('dist/app/renderer/renderer.js', 'Renderer bundle (esbuild)');
    allGood &= checkFile('dist/app/renderer/index.html', 'HTML entry');
    allGood &= checkFile('dist/app/renderer/assets/vad.worklet.bundle.min.js', 'VAD worklet');
    allGood &= checkFile('dist/app/renderer/assets/silero_vad.onnx', 'VAD model');
    allGood &= checkFile('dist/app/renderer/assets/silero_vad_legacy.onnx', 'VAD legacy model');
  } else {
    allGood = false;
  }
